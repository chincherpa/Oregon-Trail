...
def shooting():
  ...
  while True:
    ...
    if keyboard.is_pressed(" "):
      if elapsed_time < 1:
        my_score = 1
        print(f"Wham! {my_score = }")
      elif elapsed_time < 2:
        my_score = 2
        print(f"POW! {my_score = }")
      elif elapsed_time < 4:
        my_score = 3
        print(f"Blam! {my_score = }")
      else:
        my_score = 4
        print(f"Bang {my_score = }")
      break
  return my_score


def illness(game_variables):
  if get_random(1, 100) < 10 + 35 * (game_variables["eating_choice"] - 1):
  ...
  elif get_random(1, 100) < 100 - (40 / 4 ** (game_variables["eating_choice"] - 1)):
...
def mountains(game_variables):
  ...
  if get_random(1, 10) > mountain_check:
    ...
    if game_variables["south_pass_flag"]:
      ...
      # only 33%
      if get_random(1, 100) < 34:
        print("You have been caught in a Blizzard in the Mountain Pass - Time and Supplies lost.")
        blizzard(game_variables)
      elif get_random(1, 100) > 10:
        print("\nWagon Damaged! - Lose time and supplies.\nThe going gets slow.")
        game_variables["supplies"] = game_variables["supplies"] - 5
        game_variables["ammunition"] = game_variables["ammunition"] - 200
        game_variables["clothing"] = game_variables["clothing"] - (get_random(1, 40))
        game_variables["mileage"] = game_variables["mileage"] - 30 - (get_random(1, 40))
      elif get_random(1, 100) > 10:
...
def blizzard(game_variables):
  ...
  while True:
    ...
    if game_variables["clothing"] < 18 + get_random(1, 3):
      game_variables = illness(game_variables)

    # Did the weather clear?
    if get_random(1, 100) < base_chance_of_blizzard_over:
...
def game_loop(game_variables):
  ...
  while True:
    ...
    print(f"You want to eat: {amount_food} food")
    if amount_food > game_variables["food"]:
      print("You can't eat that well.")
    elif input_x == 2 and game_variables["ammunition"] < 39:
      print("TOUGH -- You need more bullets to go hunting.")
    else:
      break
  game_variables["food"] = game_variables["food"] - amount_food
  game_variables["mileage"] = game_variables["mileage"] + 200 + (
      game_variables["animals"] - 220) / 5 + get_random(1, 10)
...
def hunting(game_variables):
  ...
  if game_variables["ammunition"] > 39:
    ...
    game_variables["ammunition"] = game_variables["ammunition"] - (get_random(1, 10) * 3)

    if my_shooting > 4:
      print("You Missed -- and your dinner got away..")
    elif my_shooting < 3:
      print("Right Between the Eyes - You got a big one!! Full bellies tonight!")
      hunted_food = 52 + (my_shooting * 6)
      print(f"Food added: {hunted_food}")
    else:
      print("Nice Shot! Right on target - Good Eatin' Tonight")
      hunted_food = 48 - (my_shooting * 6)
      print(f"Food added: {hunted_food}")
  else:
    print("You need more bullets to go hunting.")

  game_variables["food"] = game_variables["food"] + hunted_food
  game_variables["mileage"] = game_variables["mileage"] - 45
  if game_variables["food"] < 10:
    dying("no_food")
...
def do_events(game_variables):
  if get_random(1, 100) < 50:
    ...
    if new_event == 1:
      ...
      game_variables["mileage"] = game_variables["mileage"] - get_random(1, 5)
    elif new_event == 2:
      ...
      game_variables["mileage"] = game_variables["mileage"] - get_random(1, 10) - 2
    elif new_event == 7:
      ...
      game_variables["mileage"] = game_variables["mileage"] - get_random(1, 10) - 5
    elif new_event == 8:
      ...
      game_variables["supplies"] = game_variables["supplies"] - get_random(1, 8) - 3
    elif new_event == 10:
      print("Lose your way in heavy fog - Time is lost")
      game_variables["mileage"] = game_variables["mileage"] - 10 - get_random(1, 5)
    elif new_event == 11:
      ...
      game_variables["mileage"] = game_variables["mileage"] - 20 - get_random(1, 20)
    elif new_event == 13:
      ...
      print("Cold Weather!!")
      if game_variables["clothing"] > get_random(1, 4) + 22:
      ...
      game_variables["ammunition"] = game_variables["ammunition"] - 200
      game_variables["supplies"] = game_variables["supplies"] - 4 - get_random(1, 3)
      game_variables["mileage"] = game_variables["mileage"] - 5 - get_random(1, 10)
    else:
...
def riders(game_variables):
  my_tactic = 0
  if get_random(1, 10) > ((game_variables["mileage"] / 100 - 4) ** 2 + 72) / (
